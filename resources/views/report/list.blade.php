    ...
    <div class="col-sm-9">
        ...
        <table class="table table-bordered no-padding marks hidden">
            <thead>
                ...
                <th width="33%">{{trans('report.mark_percent')}}</th>
            </thead>
            <tbody>

            </tbody>
        </table>
        <table class="table table-bordered no-padding attendances-sum hidden">
            <thead>
            <th width="33%">{{trans('report.sum')}}</th>
            <th width="33%">{{trans('report.percentage')}}</th>
            <th width="33%">{{trans('report.attendance_type')}}</th>
            </thead>
            <tbody>
		...
		$(document).ready(function () {
            ...
            function getMarks(subject,semester)
            {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $.each(response, function (key,value) {
                            $('.marks tbody').append('<tr><td>' + value.date +'</td><td>' + value.mark_type +'</td><td>' + value.mark_value +'</td><td>' + value.mark_percent +'</td></tr>');
                        });
                    }
                });
            }
            function getAttendances(subject,semester)
            {
                ...
                $('.attendances-sum tbody').empty();
                $('.attendances-sum').removeClass('hidden');
                var subject_id = subject.data('id');
                var semester_id = semester.data('id');
                $.ajax({
                    type: "GET",
                    ...
                    data: {_token: '{{ csrf_token() }}', 'subject_id':subject_id, 'semester_id':semester_id },
                    success: function (response) {
                        $.each(response.attendances, function (key,value) {
                            $('.attendances tbody').append('<tr><td>' + value.date +'</td><td>' + value.hour +'</td><td>' + value.attendance_type +'</td></tr>');
                        });
                        $.each(response.sum_attendance, function (key,value) {
                            $('.attendances-sum tbody').append('<tr><td>' + value.sum +'</td><td>' + value.percentage
                                +'</td><td>' + value.attendance_type +'</td></tr>');
                        });
                    }
                });
            }
